...
function reduce(state = defaultState, action) {
  switch (action.type) {
    case ACTION_TYPE.CACHE_TEMPLATE:
      {
        const {constructor, templates, token} = action.component;
        ...
        _.set(state.template, token, templates);
        if (typeof state.template[constructorName] === 'undefined') {
          _.set(state.template, constructorName, templates);
        } else {
          const target = state.template[constructorName];
          state.template[constructorName] = _.union(target, templates);
        }
