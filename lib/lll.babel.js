...
async function lll(/* ...components, opts = {} */) {
  ...
  // for json server
  const json = __.chain(page.items)
                 .filter(t => t.opts.json)
                 .map(template => {
                   return {
  ...
  const jsonFile = new File({
    base: config.base,
    path: path.join(config.base, 'db.json'),
...
async function bundleTemplate(components) {
  ...
  __.forEach(components, (component, idx) => {
    ...
    // Sort templates
    if (component.order && typeof component.order === 'function') {
      component.templates = component.order(component.templates);
    }
