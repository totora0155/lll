...
export default class Component extends ComponentBase {
  ...
  loadTemplates(glob) {
    return new Promise((resolve, reject) => {
      ...
      glob.on('match', filePath => {
        ...
        if (Object.keys(store.getState().template).length) {
          const cachedTemplate = store.getState().template[absFilePath];
  ...
  cacheCloudTemplates(cloudName) {
    ...
    _.forEach(this.templates, template => {
      const words = template.data[cloudName];
      if (!words) {
        return;
      }
