...
export default class Component extends ComponentBase {
  ...
  createAtomTemplate() {
    const files = glob.sync(path.resolve(__dirname, '../templates/atom.xml'));
    if (!files.length) {
      throw new Error('atom.xml template was not found');
    }
    const contents = fs.readFileSync(files[0], 'utf-8');
    if (this.opts.atom === 'isolate') {
      const base = glob2base(this.glob);
      const data = {
        name: 'atom',
      ...
      const template = createTemplate({
        ...
        data,
        opts: this.opts,
        token: getToken(`atom:${this.token}`)
      });
